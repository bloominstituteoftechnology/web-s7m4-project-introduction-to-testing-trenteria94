...
describe('Module 4 Project Tests', () => {
  describe('English Language', () => {
   ...
   let texts = getEntriesByKeyPrefix(txt.en, 'TEXT')
    ...
    let label = getEntriesByKeyPrefix(txt.en, 'LABEL')
    for (let [key, val] of label) {
      test(`${key} is visible`, () => {
        render(<App />)
  ...
  describe('Spanish Language', () => {
    let spanishtexts = getEntriesByKeyPrefix(txt.en, 'TEXT')
    for (let [key, val] of spanishtexts) {
      test(`${key} is visible`, () => {
        render(<App />)
        expect(screen.getByText(val)).toBeVisible
      })
    ...
    let spanishlabel = getEntriesByKeyPrefix(txt.en, 'LABEL')
    for (let [key, val] of spanishlabel) {
      test(`${key} is visible`, () => {
        render(<App />)
        expect(screen.getByLabelText(val)).toBeVisible
      })
